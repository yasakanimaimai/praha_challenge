...
この法則を守ると、モジュール間の結合度が下がるため、修正の影響が最小限で済んだりバグが発生する可能性が下がる。
回避策として getter などを用意して実装を隠蔽する方法がある。
ただし適用しすぎると、過度な抽象化になりコードが冗長になったり、可読性が落ちる。
...
getter/setter をつけるときは、利用する側が必要な値をなるべくクラス内のメソッドで提供する方がいい。
