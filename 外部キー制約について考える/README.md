...
- 参照整合性が保証されるが、アプリケーションコードから操作対象のデータを判断できないため、気を付けないと意図しない更新・削除を行なってしまう
  - 複数の子を持つ親テーブルに対して`ON DELETE`を使用するのは特に注意が必要
- アプリケーション側の実装コストが低い
- 選択すべきケース
  - `ON DELETE`については、滅多に採用すべきケースはない
    - 子レコードが大量に紐づいている場合に、それら全てが自動的に削除される恐怖
    - 広範囲にロックがかかる事によるパフォーマンス悪化が懸念事項
  - `ON UPDATE`については、親レコードと子レコードの関係が強く、常に親に紐づく場合は使用していいと思う
    ...
    → (メンターセッションにて)確かにサロゲートキーを参照キーにしているなら、必要なケースはあまりなさそう
    ただサロゲートキーはそのシステム内では一意であるだけなので、システム統合の時などを想定して最初から`CASCADE`にしておくのもあり。
    設定しておいて困ることはなさそう。
...
#### Q2. MySQLとPostgreSQLには、親レコードを持つ子レコードの登録・更新・削除時のロックの取り方に違いがある。この違いとは何か
